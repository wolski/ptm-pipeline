...
cat("Loaded", nrow(ptm_data$get_data()), "normalized PTM site abundance measurements\n")
```
...
ptm_data$set_data(dplyr::inner_join(
  ptm_data$get_data(), tot_d,
  by = c("Name", "protein_Id"),
  suffix = c(".site", ".total")
))

ptm_data$set_data(ptm_data$get_data() |>
  dplyr::mutate(ptm_usage = normalized_abundance.site - normalized_abundance.total))

cat("Merged data contains", nrow(ptm_data$get_data()), "measurements\n")
```

```{r correctDataWrite}
ptm_data$get_config()$set_response("ptm_usage")
wide <- ptm_data$to_wide()
...
ptm_data$get_config()$get_response()
ptm_data$set_data(ptm_data$get_data() |>
